...
export function MeetingForm({ onDatesChange }: { onDatesChange?: (dates: number[]) => void }) {
  ...
  const handleSubmit = async () => {
    const response = await fetch('/api/events', {
      method: 'POST',
      body: JSON.stringify({ title: meetingName, description: 'No description', timeOptions: [] }),
    });
  ...
  return (
    <div className="flex flex-col h-full p-8 bg-white min-w-[350px] max-w-[400px] w-full">
      <div className="mb-8 flex items-end gap-2">
        ...
        <span className="font-bold text-xl text-[#3d5c2c]">tomeeto tomahto</span>
      </div>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 flex-1">
        ...
        />
        <Input placeholder="Meeting Description" className="" />
            ...
            <TabsContent value="days">
              <div className="flex gap-2 flex-wrap">
                {["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map(day => (
                  <Button key={day} variant="outline" className="px-3 py-1 text-xs">{day}</Button>
                ))}
              </div>
            </TabsContent>
          </Tabs>
        </div>
        <Button className="bg-[#e5722a] hover:bg-[#d45e13] text-white mt-4 self-start font-bold">LET'S MEET!</Button>
      </form>
    </div>
  );
} 